...
// FECHA: 07/05/2023
// EMAIL: alu0101564630@ull.edu.ees
// VERSION: 1.0
...
maze_t::solve() {
  ...
  return solve_(i_start_, j_start_, camino_);
}
...
bool maze_t::solve_(const int i, const int j, dll_t<pair_t<int>>& camino_) {
  // CASO BASE:
  ...
  for (int k = 0; k < 4; k++) {
    if (is_ok_(i + i_d[k], j + j_d[k])) {
      if (solve_(i + i_d[k], j + j_d[k], camino_)) {
        pair_t<int>* pointer = new pair_t<int>(i + i_d[k], j + j_d[k]);
        camino_.push_front(new dll_node_t<pair_t<int>>(*pointer));
