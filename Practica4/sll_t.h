...
// COMENTARIOS: Archivo de cabecera con la definición de la clase sll_t
...
template <class T> void sll_t<T>::reverse() {
  assert(!empty());
  sll_node_t<T>* prev = NULL;
  sll_node_t<T>* current = head_;

  while (current != NULL) {
    sll_node_t<T>* next = current->get_next();
